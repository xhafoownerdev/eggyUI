...
--------------------------------------------------------------------
local function Ensure_Normal_Cursor(Force_Show)
	-- Clear custom icon + force-show Roblox default while UI is open.
	pcall(function()
...
function Library:Create_Window(Options)
	...
	-- Force-show default cursor + unlock mouse every frame while menu is open
	Connect(RunService.RenderStepped, function()
		if Window_Frame.Visible then
			pcall(function()
				UserInputService.MouseBehavior = Enum.MouseBehavior.Default
			end)
			Ensure_Normal_Cursor(true)
		end
	end)
